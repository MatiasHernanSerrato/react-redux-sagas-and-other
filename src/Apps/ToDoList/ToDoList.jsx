...
class ToDoList extends React.Component {
  ...
  handleOnClick = () => {
    ...
    if (!this.state.tasks) this.setState({ tasks: [] });
    this.setState({ input: "" });
    !!this.state.tasks && this.state.tasks.push(newTask);
  ...
  render() {
    return (
      <div className={"containerTodoList"}>
        ...
        <p>
          also this Component has been created as a Class Component which
          extends from React
        </p>
          ...
          <div className="taskViewer">
            {this.state.tasks &&
              this.state?.tasks?.map((task, key) => (
                <div key={key.toString()} className="taskContainer">
                  <input
                  ...
                  />
                  <label for={key.toString()}>{task.text}</label>
                </div>
              ))}
          </div>
        </div>
